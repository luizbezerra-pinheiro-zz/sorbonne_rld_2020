...
if __name__ == '__main__':
    ...
    for i in range(episode_count):
        obs = env.reset()  # initialiser l'état initial s_0
        ...
        while True:
            ...
            last_action = action
            last_state = state
            obs, reward, done, _ = env.step(action)  ## New state, obs, reward
            rsum += reward
            ...
            q[last_state][last_action] = q[last_state][last_action] + \
                                         lr * (reward + gamma * q[state][action] - q[last_state][last_action])
